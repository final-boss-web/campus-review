...
import { ShieldAlert, Sparkles, KeyRound, Mail, CheckCircle, ArrowLeft } from 'lucide-react';
...
export const VerifyRegistration = () => {
  ...
  const handleResend = async () => {
    ...
    try {
      const { data } = await api.post('/auth/resend-otp', { email, type: 'register' });
    ...
    <div className="min-h-[80vh] flex flex-col justify-center items-center px-4 py-12 relative bg-[#0D0D1A]">
      ...
      <div className="w-full max-w-md overflow-hidden bg-[#15152E] border border-[#2A2A3D] rounded-2xl shadow-xl transition-all duration-300 relative">
        ...
        <div className="p-4 pb-0">
          <Link to="/register" className="inline-flex items-center text-xs text-slate-400 hover:text-white transition">
            <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Back to Register
          </Link>
        </div>
